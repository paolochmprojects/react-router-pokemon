...
function PokemonCard({ data, pokeFav }) {
    ...
    return (<>
        <div className="w-56 mx-auto mt-16">
            ...
            <img src={imgUrl} alt={`pokemon-${id}`} />
            <div className="flex gap-2 justify-center flex-wrap capitalize">
                {types.map((type, index) => {
                    return <div key={`type-${index}`}
                        className="text-white font-bold font-poppins py-2 px-3 rounded-full"
                        style={{ background: pokeTypes[type] }}>
                        {type}
                    </div>
                })}
            </div>
            <div className="flex justify-center my-9 h-28 text-gray-500">
            ...
            {!favorite ? <fetcher.Form action="/favorites/add" method="POST">
            ...
            <fetcher.Form action={`/favorites/${favorite.id}/remove`} method="POST">
